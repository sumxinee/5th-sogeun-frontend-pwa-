  ...
  const handleSearch = async (isNewSearch = true) => {
    ...
    try {
      ...
      if (isNewSearch) {
        ...
        setOffset(20); // 다음 요청을 위해 offset 20으로 설정
      } else {
        // 더 보기: 기존 리스트 뒤에 추가
        setResults((prev) => [...prev, ...mappedTracks]);
        setOffset((prev) => prev + 20); // 다음 요청을 위해 offset 증가
      }
    } catch (err) {
      console.error(err);
    }
              ...
              {results.length > 0 ? (
                <>
                  ...
                  <div className="flex justify-center pt-4 pb-8">
                    <button
                      onClick={() => handleSearch(false)} // false: 기존 목록 유지하며 추가 로드
