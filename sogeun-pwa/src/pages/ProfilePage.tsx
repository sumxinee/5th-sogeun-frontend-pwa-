...
export default function ProfilePage() {
  ...
  return (
    // 전체 배경을 감싸는 div
    <div className="clean-profile-bg" style={{ 
      paddingTop: '60px', 
      paddingBottom: '120px', 
      display: 'flex', 
      ...
      {/* 1. 상단 핸들 */}
      <div style={{ width: '100%', paddingLeft: '24px', marginBottom: '20px', alignSelf: 'flex-start' }}>
        <h1 style={{ fontSize: '20px', fontWeight: '900', color: '#333333', margin: 0, fontFamily: 'sans-serif' }}>
          {userData.handle}
        </h1>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', width: '100%', alignItems: 'center', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: '4px' }}>
          <span style={{ fontSize: '22px', fontWeight: '900', color: '#333333' }}>{userData.nickname}</span>
          <span style={{ fontSize: '13px', color: '#666666', fontWeight: '500' }}>{userData.location}</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '3px' }}>
           ...
           <span style={{ fontSize: '14px', fontWeight: '700', color: 'rgba(255,255,255,0.9)', textShadow: '0 1px 2px rgba(0,0,0,0.1)' }}> 
             0+0 · 한로로 
           </span>
        </div>
      </div>
      ...
      {/* 4. 액션 버튼들 */}
      <div style={{ display: 'flex', width: '90%', gap: '13px', marginTop: '-7px' , marginBottom: '20px' }}>
      ...
      <div className="level-card" style={{ 
        ...
        <div className="level-header" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', fontSize: '14px', fontWeight: '700', color: '#374151' }}>
          <span>레벨 {userData.level}</span>
          <span style={{color: '#6B7280'}}>{userData.likesCurrent} / {userData.likesMax} likes</span>
        </div>
        <div style={{ width: '100%', height: '8px', backgroundColor: 'rgba(255,255,255,0.5)', borderRadius: '10px', overflow: 'hidden' }}>
          <div 
        ...
        <div className="level-footer" style={{ marginTop: '10px', fontSize: '12px', color: 'white', fontWeight: '600' }}>
      ...
      {/* 6. 좋아요 누른 노래 */}
      <div style={{ width: '90%' }}>
        <h3 style={{ fontSize: '16px', fontWeight: '800', color: 'white', marginBottom: '12px', textShadow: '0 1px 2px rgba(0,0,0,0.1)' }}>좋아요 누른 노래</h3>
        <div className="song-list-row" style={{ display: 'flex', gap: '12px', overflowX: 'auto', paddingBottom: '10px' }}>
          {userData.likedSongs.map((src, index) => (
            <div key={index} style={{ flexShrink: 0, width: '100px', height: '100px', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 10px rgba(0,0,0,0.1)' }}>
              <img src={src} alt={`album-${index}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
          ))}
        </div>
      </div>
