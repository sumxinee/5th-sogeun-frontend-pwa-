...
export default function ProfilePage() {
  ...
  const fetchSogeunData = async () => {
    try {
      ...
      if (response.status === 200 && response.data) {
        const fetchedLikes =
          response.data.totalSogeun || response.data.likesCount || 0;
        ...
        if (
          ...
          const songImages = response.data.songs.map(
            (song: any) => song.imageUrl || song.cover,
          );
  ...
  const progressPercent = Math.min(
    (userData.likesCurrent / userData.likesMax) * 100,
    100,
  );
    ...
    >
      ...
      <div className="level-card">
        ...
        <div className="level-footer">
          다음 레벨까지{" "}
          {userData.likesMax - userData.likesCurrent > 0
            ? userData.likesMax - userData.likesCurrent
            : 0}{" "}
          likes 남음
        </div>
      </div>
